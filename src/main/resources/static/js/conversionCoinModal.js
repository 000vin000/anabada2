document.getElementById("toCashList").addEventListener("click", function () {
	openConversionToCashModal();
});

// 신청 내역 열기
function openConversionToCashModal() {
...
// 서버에서 데이터 받아오기
function getToCashData() {
	const token = localStorage.getItem('Token');
	
	fetch('/api/coin/conversionToCashList', {
		method: 'GET',
		headers: {
	...
	.then(data => {
		populateConversionToCashHistory(data); 
	})
	.catch(error => {
...
// 데이터 보여주기
function populateConversionToCashHistory(data) {
	const tbody = document.querySelector("#conversionModal tbody");
		...
		if (Array.isArray(data.conList)) {
			...
			document.querySelectorAll('.toCashCancel').forEach(button => {
