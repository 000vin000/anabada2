...
function loadCurrentCashCoin() {
	...
	.then(data => {
		...
		const userInfoHTML = `
			...
			<div><span class="userInfoLabel">경고 횟수</span><span class="userInfoValue"><a class="goToMyWarnPage" onclick="openWarnWindow('받은 경고', '/myWarnList')" style="cursor: pointer;">${user.userWarnCnt} 회</a></span></div>
		`;
